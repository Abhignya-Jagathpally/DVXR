"""dvxr.experiments.llm_representation_probe — EXPERIMENTAL LLM-in-the-predictive-path probe.

NOT FOR CLINICAL INFERENCE. This is a research probe, quarantined out of the product path. It
is the project's honest NEGATIVE result (the frozen-LLM soft-prompt reader is the weakest
predictor and never beats the tuned baselines); it exists so the experiment is reproducible, not
because the LLM predicts clinical risk. In the product the LLM is explanation-only
(`dvxr.llm.insight`). Do not import this from `dvxr.serve`, `dvxr.realtime`, the request
lifecycle, or any product-claim surface. Formerly `dvxr.llm.predictor` (kept as a thin
deprecation shim for existing bench/experiment importers).

--- original design notes ---
The LLM in the PREDICTIVE path (Option 3: soft-prompt fusion).
...
#: Hard markers that this module is a research probe, never a clinical predictor.
EXPERIMENTAL_ONLY = True
NOT_FOR_CLINICAL_INFERENCE = True
...
def _modality_quant(task, seed: int, d_code: int) -> Dict[str, np.ndarray]:
    ...
    # VQ training epochs. Default raised 8 -> 30 after a CI-backed win: on wesad_stress the LLM-rep
    # held-out AUROC rose 0.716 -> 0.843 (non-overlapping CIs) purely from better-trained, less-collapsed
    # codebooks (docs/IMPROVEMENT_EXPERIMENT.md). Override with DVXR_VQ_EPOCHS.
    try:
    ...
    for m in task.modalities:
        ...
        enc = VQBiosignalEncoder(embedding_dim=d_code, hidden_dim=32, n_layers=1,
                                 n_heads=2, epochs=epochs, codebook_size=64, seed=seed, simvq=simvq)
