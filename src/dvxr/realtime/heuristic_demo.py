"""dvxr.realtime.heuristic_demo — FusedRealtimeMonitor, a HEURISTIC streaming DEMO (not the model).

EXPERIMENTAL / DEMO ONLY. This monitor is a transparent heuristic: stress is a recent-vs-global
signal-mean z-score, glucose is a linear extrapolation with a FIXED interval, and complication-like
indicators are hard-coded thresholds. It is NOT the calibrated NeuroGlycemic Sentinel predictor — it
does not run the trained CGM excursion heads, quality-aware fusion, or patient-specific uncertainty.
Use it for streaming-UI illustration; never present its output as a validated risk prediction. The
production predictive path is ``dvxr.prediction`` behind the Generate lifecycle.

``.update()`` / ``.reset()`` stay compatible with ``RealtimeMonitor``. Deterministic.
"""
from __future__ import annotations

#: Guards against this heuristic demo being mistaken for the calibrated product (mirrors the
#: llm_representation_probe pattern). Serving/clinical paths must not depend on this module.
